...
- Console admin para intervencoes ao vivo

## Requisitos de mapa e escala

Com mapa maior, comandos admin devem aceitar coordenadas ate `size-1`.
...
Spawn manual de objetos por tipo e modo.
...
- `POST /admin/spawn`
- `POST /admin/world/patch`
- `POST /admin/agent/{id}/profile`
...
Dependencias: auth admin + endpoints de patch no world.
