...
Esta task define a implementacao de `F14` no modo `warfare/hybrid`.

Objetivo central:

- Combate por arremesso de objetos

## Requisitos de mapa e escala

Mapa maior exige tuning de alcance e trajetoria para nao virar ataque global.
...
Objetos: `throwable_stone`, `throwable_bottle`, `cover`.
...
Dependencias: schema de acao + engine de dano/range.
