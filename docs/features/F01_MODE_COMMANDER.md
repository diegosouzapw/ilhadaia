...
Esta task define a implementacao de `F01` no modo `survival`.

Objetivo central:

- Controle de agente por linguagem natural com comando assistido

## Requisitos de mapa e escala

Nao exige novo objeto fisico obrigatorio; usa mapa ampliado para rotas longas e comandos multi-etapa.
...
Usa entidades atuais e pode criar marcador visual `command_waypoint` no frontend.
...
- `POST /agents/{id}/command/cancel`
- `GET /agents/{id}/command`
...
Dependencias: F03 (observabilidade) e fundacao de mapa ampliado.
